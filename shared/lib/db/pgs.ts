...
export async function getCaretakerPGs(caretakerId: string): Promise<PG[]> {
  try {
    const q = query(collection(db, "pgs"), where("caretakerId", "==", caretakerId));
    const snapshot = await getDocs(q);
    if (snapshot.empty) return [];
    return snapshot.docs.map((d) => ({ id: d.id, ...d.data() })) as PG[];
  } catch (error) {
    console.error("Error fetching PGs by caretaker:", error);
    return [];
  }
}
