...
vec3 getColor(vec2 p){
    ...
    vec3 baseColor = vec3(0.9) * middle + (sum + middle) * 0.15 * vec3(1.0) * bar;
    return baseColor;
}
...
export function FaultyTerminalBackground({
  ...
  useEffect(() => {
    // Make dpr strictly 1 to ensure smooth performance on low-end devices and high-dpi displays
    const dynamicDpr = 1;

    const ctn = containerRef.current;
    if (!ctn) return;

    const renderer = new Renderer({ dpr: dynamicDpr });
