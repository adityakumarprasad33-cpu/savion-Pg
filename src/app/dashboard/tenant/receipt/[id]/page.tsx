...
export default function ReceiptPage() {
  ...
  const statusConfig = {
    ...
    rejected: { icon: <XCircle className="w-5 h-5 text-red-600" />, label: "Rejected ❌", color: "text-red-700 bg-red-50 dark:bg-red-950/30 border-red-200" },
  };
  const s = statusConfig[payment.status];

  return (
    <div className="min-h-screen bg-slate-50 dark:bg-zinc-800/50 py-10 px-4 print:bg-white dark:bg-zinc-900 print:p-0">
      ...
      {/* Receipt Card */}
      <div className="max-w-2xl mx-auto bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-700 rounded-2xl shadow-lg dark:shadow-zinc-900/50 overflow-hidden print:shadow-none print:border-0">
        {/* Header */}
          ...
          <div className="divide-y divide-slate-100">
            {[
              ...
              <div key={row.label} className="flex justify-between items-center py-3 gap-4">
                <span className="text-sm text-slate-500 dark:text-slate-400 shrink-0 w-44">{row.label}</span>
                <span className={`text-sm font-semibold text-slate-900 dark:text-slate-100 text-right ${row.mono ? "font-mono" : ""}`}>{row.value}</span>
              </div>
            ))}

            {/* Amount — highlighted */}
            <div className="flex justify-between items-center py-4 gap-4 bg-slate-50 dark:bg-zinc-800/50 -mx-8 px-8 mt-2">
              <span className="text-base font-bold text-slate-700 dark:text-slate-300">Amount Paid</span>
        ...
        {/* Footer */}
        <div className="px-8 py-5 border-t bg-slate-50 dark:bg-zinc-800/50 text-center print:bg-white dark:bg-zinc-900">
