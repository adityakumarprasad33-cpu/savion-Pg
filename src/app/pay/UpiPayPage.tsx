...
export default function UpiPayPage() {
  ...
  const handleConfirm = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Create the payment record. Owner verification is required before confirmation.
      await submitPayment({
      ...
      await createNotification({
        ...
        message: `Your payment of Rs ${session.amount.toLocaleString("en-IN")} for ${session.month} at ${session.pgName} was submitted. The owner will verify UTR ${cleanUtr}.`,
        type: "booking",
      });

      // Notify OWNER - verification required
      await createNotification({
        userId: session.ownerId,
        title: "Payment Awaiting Verification",
        message: `${session.tenantName} submitted Rs ${session.amount.toLocaleString("en-IN")} for ${session.month} at ${session.pgName}. Verify UTR ${cleanUtr} before confirming the booking.`,
        type: "booking",
      });
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-green-50 to-emerald-100 flex items-center justify-center p-6">
        <div className="bg-white dark:bg-zinc-900 rounded-3xl shadow-2xl dark:shadow-zinc-900/60 p-10 max-w-sm w-full text-center">
          ...
          <h2 className="text-2xl font-black text-green-700 mb-2">Payment Submitted</h2>
          <p className="text-muted-foreground text-sm mb-1">
            Rs {session!.amount.toLocaleString("en-IN")} for <strong>{session!.month}</strong> at <strong>{session!.pgName}</strong> is waiting for owner verification.
          </p>
          <div className="mt-4 bg-green-50 border border-green-200 rounded-xl p-3 text-left">
            <p className="text-xs text-green-800 font-semibold">Payment record created safely</p>
            <p className="text-xs text-green-700 mt-0.5">Both you and the owner have been notified</p>
            <p className="text-xs text-green-700 mt-0.5">Booking confirms after owner verification</p>
          </div>
