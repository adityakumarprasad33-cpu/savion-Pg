...
import { Check, X, Eye, ShieldCheck, AlertCircle, ArrowLeft, Filter, RefreshCw } from "lucide-react";
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-black text-slate-900">User Verifications</h1>
              <p className="text-slate-500">Review and manage tenant identity documents.</p>
            </div>
          </div>
              ...
              {filtered.map((r) => (
                <tr key={r.id} className="hover:bg-slate-50 transition-colors">
                  <td className="px-6 py-4">
                    <p className="font-bold text-slate-900">{r.fullName || "Anonymous"}</p>
                    <p className="text-xs text-slate-500">{r.userEmail}</p>
                    <span className="inline-block mt-1 text-[10px] bg-slate-100 px-1.5 py-0.5 rounded font-mono">{r.id}</span>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex gap-2">
                      <button onClick={() => setPreviewImg(r.idUrl)} className="p-2 bg-slate-100 rounded-lg hover:bg-slate-200 transition-colors" title="View ID Card">
      ...
      {previewImg && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-8" onClick={() => setPreviewImg(null)}>
          <div className="relative max-w-2xl max-h-[80vh]">
            <button onClick={() => setPreviewImg(null)} className="absolute -top-3 -right-3 w-8 h-8 bg-white rounded-full shadow-lg flex items-center justify-center z-10">
              <X className="w-4 h-4" />
            </button>
            <img src={previewImg} alt="Document" className="max-w-full max-h-[80vh] rounded-xl shadow-2xl object-contain" />
          </div>
        </div>
      )}
    </div>
...
function Loader2Icon() {
  return <div className="w-4 h-4 border-2 border-slate-300 border-t-slate-600 rounded-full animate-spin" />;
}
