...
function friendlyError(code: string): string {
  const map: Record<string, string> = {
    ...
    "auth/invalid-phone-number": "Invalid phone number. Please include the country code (e.g. +91).",
...
export default function LoginPage() {
  ...
  const onEmailSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Removed manual handleRoute here because onAuthStateChanged catches it automatically
    } catch (e: any) {
      setError(friendlyError(e.code));
      setLoading(false);
    }
  ...
  return (
    <div className="mx-auto flex w-full flex-col justify-center space-y-8 sm:w-[450px] relative mt-12 mb-12">
      ...
      <div className="grid gap-6 bg-card p-6 sm:p-8 rounded-2xl border shadow-sm dark:shadow-slate-900/50">
        {/* Error Banner */}
        {error && (
